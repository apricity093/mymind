...
async def lifespan(app: FastAPI):
    ...
    _skill_manager = SkillManager(
        root_dir=skills_dir,
        max_prompt_chars=int(os.getenv("mymind_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    _skill_manager.load()
...
async def _cli():
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("mymind_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("mymind_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
